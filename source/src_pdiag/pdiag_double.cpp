	...
	double* ekb) // eigenvalues for each k-point and band
{
	...
	{
		ModuleBase::timer::tick("Diago_LCAO_Matrix","scalapack_gvx_solve");
...
//			throw std::domain_error("INPUT.new_dm must be 1. "+ModuleBase::GlobalFunc::TO_STRING(__FILE__)+" line "+ModuleBase::GlobalFunc::TO_STRING(__LINE__));
	...
	} if(GlobalV::KS_SOLVER=="cusolver")
	{
		...
		cuGather_complex(ch_mat, htot);
		cuGather_complex(cs_mat, stot);
...
void Pdiag_Double::cuGather_complex(std::complex<double> *mat_loc, std::complex<double> *mat_glb){
